...
import ca.sfu.isc.grouper.ws.coresoap.WsExternalEmailSubjectSaveResult.WsExternalEmailSubjectSaveResultCode;
...
/**
 * <pre>
 * results for the External Email Subject save call.
 * 
 * result code:
 ...
 * @author mchyzer
 */
public class WsExternalEmailSubjectSaveResults implements WsResponseBean, ResultMetadataHolder {

  /**
   * result code of a request
   */
  public static enum WsExternalEmailSubjectSaveResultsCode implements WsResultCode {
     ...
     * @param statusCode
     */
    private WsExternalEmailSubjectSaveResultsCode(int statusCode) {
   ...
   * logger 
   */
  private static final Log LOG = LogFactory.getLog(WsExternalEmailSubjectSaveResults.class);

  /**
   * prcess an exception, log, etc
   * @param wsExternalEmailSubjectSaveResultsCodeOverride
   * @param theError
   * @param e
   */
  public void assignResultCodeException(
      WsExternalEmailSubjectSaveResultsCode wsExternalEmailSubjectSaveResultsCodeOverride, String theError, Exception e) {

    if (e instanceof WsInvalidQueryException) {
      wsExternalEmailSubjectSaveResultsCodeOverride = GrouperUtil.defaultIfNull(
          wsExternalEmailSubjectSaveResultsCodeOverride, WsExternalEmailSubjectSaveResultsCode.INVALID_QUERY);
      //      if (e.getCause() instanceof GroupNotFoundException) {
      //        wsGroupSaveResultsCodeOverride = WsGroupSaveResultsCode.GROUP_NOT_FOUND;
      //      }
      //a helpful exception will probably be in the getMessage()
      this.assignResultCode(wsExternalEmailSubjectSaveResultsCodeOverride);
      ...
      wsExternalEmailSubjectSaveResultsCodeOverride = GrouperUtil.defaultIfNull(
          wsExternalEmailSubjectSaveResultsCodeOverride, WsExternalEmailSubjectSaveResultsCode.EXCEPTION);
   ...
   */
  public void assignResultCode(WsExternalEmailSubjectSaveResultsCode externalSubjectSaveResultsCode) {
   ...
   */
  public boolean tallyResults(String theSummary, GrouperVersion clientVersion) {
    ...
    if (this.getResults() != null) {
      ...
      int successes = 0;
      int failures = 0;
      for (WsExternalEmailSubjectSaveResult wsExternalEmailSubjectSaveResult : this.getResults()) {
        boolean theSuccess = "T".equalsIgnoreCase(wsExternalEmailSubjectSaveResult.getResultMetadata()
            .getSuccess());
        if (theSuccess) {
          successes++;
        } else {
      ...
      if (!successOverall || failures > 0) {
        for (WsExternalEmailSubjectSaveResult wsExternalSubjectSaveResult : this.getResults()) {
          if (GrouperUtil.booleanValue(wsExternalSubjectSaveResult.getResultMetadata().getSuccess(),
              true)) {
            wsExternalSubjectSaveResult.assignResultCode(WsExternalEmailSubjectSaveResultCode.EXCEPTION, clientVersion);
            failures++;
          }
        }
      }
      ...
      if (failures > 0) {
        ...
        this.assignResultCode(WsExternalEmailSubjectSaveResultsCode.PROBLEM_SAVING_EXERNAL_SUBJECTS);
        //this might not be a problem
        LOG.warn(this.getResultMetadata().getResultMessage());

      } else {
        this.assignResultCode(WsExternalEmailSubjectSaveResultsCode.SUCCESS);
      }
    } else {
      //none is not ok
      this.assignResultCode(WsExternalEmailSubjectSaveResultsCode.INVALID_QUERY);
   ...
   */
  private WsExternalEmailSubjectSaveResult[] results;
   ...
   * @return the results
   */
  public WsExternalEmailSubjectSaveResult[] getResults() {
    return this.results;
  }
   ...
   */
  public void setResults(WsExternalEmailSubjectSaveResult[] results1) {
